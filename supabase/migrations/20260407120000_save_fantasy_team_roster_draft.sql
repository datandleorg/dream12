-- Allow save_fantasy_team with null captain + vice_captain for roster-only persistence (resume captain step later).
-- When both are null: validate XI only; on update, do not overwrite existing captain columns.
