-- Profile avatars: column + view + season_leaderboard output.

alter table public.profiles
  add column if not exists avatar_url text;

comment on column public.profiles.avatar_url is
  'HTTPS URL of user profile image (e.g. DigitalOcean Spaces); null if unset.';
...
create function public.season_leaderboard(
  p_season_id bigint,
...
comment on function public.season_leaderboard(bigint, int, int) is
  'Aggregates user_teams points across contests in a season whose matches are completed and scoring-finalized; includes username, avatar_url, totals, and efficiency metrics.';
