-- Reverts logic from migration 20260329200000_live_contest_mutations.sql.
-- Restores: lock when now() >= start_time - 1 minute for ALL matches (no live bypass,
-- no explicit "match has finished" for completed — completed matches are still blocked
-- by the time rule if start_time is in the past).
--
-- Run in SQL editor or: psql ... -f supabase/scripts/revert-live-contest-mutations.sql
-- Optional: copy into a new dated migration if you want this applied via `supabase db push`.
