...
--
-- After running: Dashboard → Settings → API → "Reload schema" (or wait ~1 min).

-- User-created contests + visibility until creator saves team + create_user_contest RPC
...
alter table public.contests
  ...
  add column if not exists prize_breakup jsonb,
  add column if not exists is_flexible boolean not null default true,
  add column if not exists gross_collected numeric(12, 2);
...
-- Replace save_fantasy_team: mark creator joined on first team insert
